...
{
	struct cherry_kiwi_adapter *p_driver_data = container_of(pnode->i_cdev, struct cherry_kiwi_adapter, m_dev_slnic);
	...
	for (channel_status_count = 2; channel_status_count < 8; channel_status_count++)
	{
		...
		//改变通道状态的值
		p_driver_data->channel_status[channel_count] = 0;
		...
		for (num_count = 0; num_count < config->m_session_num; num_count++)
		{
			...
			WRITE_REG(p_driver_data->m_mem_base0, session_s_mac_addr_l, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_s_mac_l_and_dh, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_d_mac_h, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_sip_addr, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_sport_addr, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_dip_addr, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_dport_addr, 0);
			WRITE_REG(p_driver_data->m_mem_base0, session_en_clt_addr, 0);
		}

		//删除通道之后，读完某个通道的数据
		rx_rec_buff.buffer = p_driver_data->m_rx_channel.m_dma_chan[channel_count].m_virt_addr_start;

		//获得需要释放节点下标
		rx_rec_buff.m_next_chunk = (p_driver_data->m_rx_seqs[channel_count].m_info.m_read_seq)&(p_driver_data->m_rx_channel.m_dma_chan[channel_count].m_node_num-1);
		//获取当前软件计数
		rx_rec_buff.m_chunk_count = p_driver_data->m_rx_seqs[channel_count].m_info.m_read_seq;
		...
		while (1)
		{
			...
			if (rx_rec_buff.buffer[rx_rec_buff.m_next_chunk].info.m_chunk_count != rx_rec_buff.m_chunk_count)
			{
				...
				p_driver_data->m_rx_seqs[channel_count].m_info.m_read_seq = rx_rec_buff.m_chunk_count - 1;
				break;
			}

			WRITE_REG(p_driver_data->m_mem_base0, SW_PKT_CUR_MEM_OFF0 + SW_PKT_CUR_MEM_OFFSET * channel_count, rx_rec_buff.m_next_chunk);

			rx_rec_buff.m_next_chunk = (rx_rec_buff.m_next_chunk + 1) & (p_driver_data->m_rx_channel.m_dma_chan[channel_count].m_node_num - 1);
...
static void do_config_session(struct hw_session_cnf cnf,struct cherry_kiwi_adapter*p_driver_data)
{
	int s_mac_addr[6] = { 0 };
	int d_mac_addr[6] = { 0 };
	...
	WRITE_REG(p_driver_data->m_mem_base0,session_s_mac_addr_l, s_mac_addr_h);
	WRITE_REG(p_driver_data->m_mem_base0,session_s_mac_l_and_dh, s_mac_l_and_dh);
	WRITE_REG(p_driver_data->m_mem_base0,session_d_mac_h, d_mac_addr_l);
	WRITE_REG(p_driver_data->m_mem_base0,session_sip_addr, ntohl(ip_str_to_num(sip)));
	WRITE_REG(p_driver_data->m_mem_base0,session_sport_addr, /*htons*/(sport));
	WRITE_REG(p_driver_data->m_mem_base0,session_dip_addr, ntohl(ip_str_to_num(dip)));
	WRITE_REG(p_driver_data->m_mem_base0,session_dport_addr, /*htons*/(dport));
	WRITE_REG(p_driver_data->m_mem_base0,session_en_clt_addr, en_ctl);
}
	...
	case SLK_IOCTL_CONFIG_CHANNEL:
		{
			...
			//判断全局通道是否占用
			if (p_driver_data->channel_status[channel_count] & 1)
				return -EFAULT;

			//配置全局通道状态，不允许其他通道来占用
			p_driver_data->channel_status[channel_count] = 1;
			...
			for (num_count = 0; num_count < config->m_session_num; num_count++)
			{
				...
				do_config_session(scnf, p_driver_data);
			}
			if (NULL != config)
			{
				kfree(config);
	...
	case IOCTL_SLN:
		{
			...
			port_num = info.port;
			ndev = p_driver_data->ndev[port_num];
