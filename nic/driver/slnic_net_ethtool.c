...
static void slnic_net_led_on(struct slk_net_priv* priv)
{
	if (priv->port == 0)
	{
		WRITE_REG(priv->m_p_slnic_data->m_mem_base0, MAC0_LED, LED_ON);
		sl_debug("Mac0 LED ON");
	}
	else if (priv->port == 1)
	{
		WRITE_REG(priv->m_p_slnic_data->m_mem_base0, MAC1_LED, LED_ON);
		sl_debug("Mac0 LED ON");
	}
}
...
static void slnic_net_led_off(struct slk_net_priv* priv)
{
	if (priv->port == 0)
	{
		WRITE_REG(priv->m_p_slnic_data->m_mem_base0, MAC0_LED, LED_OFF);
		sl_debug("Mac0 LED OFF");
	}
	else if (priv->port == 1)
	{
		WRITE_REG(priv->m_p_slnic_data->m_mem_base0, MAC1_LED, LED_OFF);
