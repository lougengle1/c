...
{
	uint32_t					m_i_rx_l_reg_addr;		/// rx队列地址寄存器
	uint32_t					m_i_rx_reg_count;		/// rx计数器地址寄存器
...
extern struct slk_rx_channel* create_rx_channel_limit_alloc(int chan_id, struct slk_rx_chanbase* base, SLK_PTR_DAM_ADDR_T dma_count, void* vis_count, SLK_PTR_PCI_DEV p_dev, unsigned long bar0, SLK_SPINLOCK_T lcl_lock, SLK_PTR_DAM_ADDR_T min_addr, int alloc_time, int use_anywall);
/// 销毁一个通道
...
/// 通知硬件读取到下标
extern int rx_channel_recover_index(struct slk_rx_mmap_head* p_head, uint64_t channel_id, unsigned long bar0_addr, int idx);
