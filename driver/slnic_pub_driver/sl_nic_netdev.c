...
static void check_recover(void)
{
	...
	//sl_debug("[g_driver_data.tx_dma_count %d] [g_driver_data.m_tx_queue->counter %d]\n", g_driver_data.tx_dma_count, g_driver_data.m_tx_queue->counter);
...
static int slnic_net_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	//err = down_interruptible(&g_driver_data.m_event_sem);
	//if (err)
	//{
	//	dev_kfree_skb(skb);
	//	up(&g_driver_data.m_event_sem);
	//	return err;
	//}
	...
	check_recover();
	err = slk_tx_mem_op_list_write((char*)priv->buff, sizeof(struct driver_to_hardware_head)+skb->len, g_driver_data.m_tx_queue);
	if (err < 0)
	{
...
//	dev->trans_start = jiffies;		/// 保存当前时间撮
//#endif
...
void slnic_netdev_check_link(struct net_device *ndev)
{
	...
	reg = READ_REG(g_driver_data.m_mem_base0, SYS_STATUS);
	//sl_debug("[reg:%x] [priv->port:%d]\n", reg,priv->port);
	...
	if (priv->port == 0)
	{
		if (reg & 0x1)
		{
			...
			{
				//sl_debug("netif_carrier_ok\n");
				netif_carrier_on(ndev);
			}
		}
		else if (netif_carrier_ok(ndev))
		{
			//sl_debug("netif_carrier_ok not\n");
...
static int slnic_get_mac_addr_regs(struct net_device *ndev,int port_num)
{
	uint32_t mac_h;
	uint32_t mac_l;
	
	mac_h = READ_REG(g_driver_data.m_mem_base0, MAC0_ADDR_H + port_num *MAC_OFFSET);
	mac_l = READ_REG(g_driver_data.m_mem_base0, MAC0_ADDR_L + port_num *MAC_OFFSET);

	//如果读到的时候空，就设置成默认值
	if ((mac_h == 0) && (mac_l == 0))
	{
		memcpy(ndev->dev_addr, "netma", 5);
		ndev->dev_addr[5] = port_num;
	}
	else
	{
...
int slnic_netdev_alloc(struct net_device **ndev_ptr,unsigned int port_num,unsigned long bar0_addr)
{
	...
	///读取mac寄存器地址并且设置mac地址	
	slnic_get_mac_addr_regs(ndev, port_num);
	...
	priv->pci_dev = g_driver_data.m_dev_pci;
	priv->regs_len = pci_resource_len(g_driver_data.m_dev_pci, 0);
	priv->msg_enable = netif_msg_init(g_driver_data.m_debug, DEFAULT_MSG_ENABLE);
	priv->tx_ring_count = SLK_FPGA_DEFAULT_TXD;
	priv->rx_ring_count = SLK_FPGA_DEFAULT_RXD;
	priv->b_napi_running =1;
	priv->m_rx_enable = 0;
	priv->m_rx.buffer = g_driver_data.m_rx_channel.m_dma_chan[priv->port].m_virt_addr_start;
...
void slk_destory_netdev(void)
{
	...
	{
		struct slk_net_priv *priv = netdev_priv(g_driver_data.ndev[i]);
		...
		napi_disable(&priv->m_sl_nic_napi);
		sl_debug("1111111\n");
		unregister_netdev(g_driver_data.ndev[i]);
		sl_debug("1111111\n");
		netif_napi_del(&priv->m_sl_nic_napi);
		sl_debug("1111111\n");
		free_netdev(g_driver_data.ndev[i]);
	}
}
